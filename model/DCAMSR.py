...
class DCA(nn.Module):
    ...
    def forward(self, x, ref):
        ...
        x_CA = (attn_CA @ v_CA).permute(0, 3, 1, 2).reshape(B, N,
                                                            C)  # (B,Head,C,C) * (B,Head,C,N) -> (B,Head,C,N) -> (B,N,C)

        #### 位置注意力
        k_shared_projected = self.E(k_shared)
        v_SA_projected = self.F(v_SA)

        attn_SA = (q_shared.permute(0, 1, 3,
                                    2) @ k_shared_projected) * self.temperature2  # (B,Head,N,C) * (B,Head,C,64) -> (B,Head,N,64)
...
class DCAMSR(nn.Module):
    def __init__(self, input_size=200, scale=1):
        super().__init__()
        in_chl = 1
        nf = 64
        n_blks = [4, 4, 4]
        ...
        self.trans_lv1 = nn.ModuleList(
            [DCAT(input_size=input_size * input_size, hidden_size=64, proj_size=64, pos_embed=i != 0) for i in
             range(depths[0])])
        self.trans_lv2 = nn.ModuleList(
            [DCAT(input_size=input_size * input_size // 4, hidden_size=64, proj_size=64, pos_embed=i != 0) for i in
             range(depths[1])])
        self.trans_lv3 = nn.ModuleList(
            [DCAT(input_size=input_size * input_size // 16, hidden_size=64, proj_size=64, pos_embed=i != 0) for i in
             range(depths[2])])

    def forward(self, ref, lr, ref_down=None, gt=None):

        # lrsr = F.interpolate(lr, scale_factor=self.scale, mode='bilinear')

        fea_lrsr = self.enc(lr)
...
if __name__ == '__main__':
    dcamsr = DCAMSR(input_size=200)
    pd_hr = torch.randn(1, 1, 200, 200)
    t2_lr = torch.randn(1, 1, 200, 200)
    t2_hr = torch.randn(1, 1, 200, 200)

    t2_sr = dcamsr(pd_hr, t2_lr)
    print(t2_sr.shape)
    loss = F.l1_loss(t2_sr, t2_hr)
    loss.backward()
