...
class VANet(nn.Module):
    def __init__(self, in_channel=32, channel_fea=32, iter_num=4):
        ...
        ## CDic layer
        self.E_x = nn.Sequential(
            *[nn.Conv2d(in_channels=self.in_channel, out_channels=self.channel_fea, kernel_size=3, padding=3 // 2), \
              nn.ReLU(), \
    ...
    def forward(self, x, y, mask):
        ...
        # x_k
        mask = mask.repeat(1, self.in_channel * 2, 1, 1)
        ...
        for i in range(self.iter_num):
            ...
            refine = self.gamma_x[i] * self.D_x(self.E_x(x_t0) - self.E_c(c_t0)) + self.lamda_1[i] * self.denoise_x(
                x_t0)  # refine
...
if __name__ == '__main__':
    mcvarnet = VANet(in_channel=32, channel_fea=32, iter_num=4)
    pd_hr = torch.randn(1, 1, 200, 200)
    t2_lr = torch.randn(1, 1, 200, 200)
    t2_hr = torch.randn(1, 1, 200, 200)
    k_space_mask = torch.randn(1, 1, 200, 200)

    t2_sr, pd_sr = mcvarnet(t2_lr, pd_hr, k_space_mask)
    print(t2_sr.shape)

    loss_t2 = F.l1_loss(t2_sr, t2_hr)
    loss_pd = F.l1_loss(pd_sr, pd_hr)
    loss = loss_t2 + 0.1 * loss_pd
    loss.backward()
